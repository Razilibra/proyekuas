...
@section('title', 'Tambah Usulan PKL')
        ...
        <div class="col-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">Tambah Usulan PKL</h6>
                </div>
                <div class="card-body">
                    <form action="{{ route('usulan_pkl.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('id_mahasiswa') is-invalid @enderror" id="id_mahasiswa" name="id_mahasiswa">
                                ...
                                @foreach($mahasiswa as $m)
                                    <option value="{{ $m->id_mahasiswa }}" {{ old('id_mahasiswa') == $m->id_mahasiswa ? 'selected' : '' }}>
                                        {{ $m->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('konfirmasi') is-invalid @enderror" id="konfirmasi" name="konfirmasi">
                                <option value="0" {{ old('konfirmasi') == '0' ? 'selected' : '' }}>Belum Dikonfirmasi</option>
                                {{-- <option value="1" {{ old('konfirmasi') == '1' ? 'selected' : '' }}>Dikonfirmasi</option> --}}
                            </select>
                            @error('konfirmasi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
